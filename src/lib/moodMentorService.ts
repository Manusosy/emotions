...
class MoodMentorService {
   ...
   * @param formData The form data containing the image file and user ID
  ...
  async uploadProfileImage(formData: FormData) {
    try {
      const response = await api.post('/api/upload-profile-image', formData, {
        headers: {
          // Don't set Content-Type here as the browser will set it with the correct boundary for FormData
        }
      });
      ...
      errorLog("Error uploading profile image:", error);
